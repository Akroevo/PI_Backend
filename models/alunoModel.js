...
const Aluno = {
  ...
  findSemCurso: () => db.query(
    `SELECT a.*, u.email FROM aluno a
     LEFT JOIN usuario u ON u.idusuario = a.usuario_idusuario
     LEFT JOIN aluno_curso ac ON ac.aluno_matricula = a.matricula
     WHERE ac.aluno_matricula IS NULL`
  ),
  ...
  findByCoordenador: (idCoordenador) => db.query(
    `SELECT DISTINCT a.*, u.email FROM aluno a
     LEFT JOIN usuario u ON u.idusuario = a.usuario_idusuario
     JOIN aluno_curso ac ON ac.aluno_matricula = a.matricula
     JOIN coordenador_curso cc ON cc.curso_idCurso = ac.curso_idCurso
     WHERE cc.coordenador_idCoordenador = ?`, [idCoordenador]
  )
};
